...
#Modified to include evolving RSC trait (4th trait) AND child function options
#Child function options: :parabola, :asymptote, or :linear
...
@everywhere function compatibility(m::Float64, f::Float64)
  1 / (1 + abs(m - f))
end

# Expected offspring count given compatibility score in [0,1]
# model can be :parabola, :asymptote, or :linear
@everywhere function offspring_mean(score::Float64, model::Symbol)
    ...
    elseif model === :linear
        ...
        error("Unknown model: $model. Must be :parabola, :asymptote, or :linear")
    end
end

#simulation function with evolving RSC trait AND child function options
# child_model can be :parabola, :asymptote, or :linear
@everywhere function sim(N,mu,var,a,rsc,tradeoff,generations,d=-1,child_model::Symbol=:parabola)
  ...
  #Preallocating results (24 columns: original 22 + MeanChildren + TotalChildren)
  dfall=zeros(generations,24)
  ...
  #maternal genome females next gen - need to allocate more space for variable children
  # Estimate max children: if all females mate and have max children (say 10), we need N*10 space
  max_children_per_female = 10  # reasonable upper bound
  max_total_offspring = N * max_children_per_female
  mgf2=zeros(max_total_offspring,20,ntraits)
  #paternal genome females next gen
  pgf2=zeros(max_total_offspring,20,ntraits)
  #maternal genome males next gen
  mgm2=zeros(max_total_offspring,20,ntraits)
  #paternal genome males next gen
  pgm2=zeros(max_total_offspring,20,ntraits)
  ...
  Stdsperm=std(mphens[:,3])
    ...
    # Track children per female and mates-to-children relationship
    children_per_female = zeros(size(fphens)[1])
    total_children_this_gen = 0
    ...
    for i in 1:size(fphens)[1]
      ...
      if mates==1
        # Calculate compatibility between female and male
        compat_score = compatibility(mphens[matesM[1],2], fphens[i,1])
        
        # Calculate expected number of children based on compatibility and child model
        expected_children = offspring_mean(compat_score, child_model)
        
        # Sample actual number of children from Poisson distribution
        n_children = rand(Poisson(max(0.1, expected_children)))  # floor at 0.1 to avoid zero
        
        # Ensure at least 1 child if mating occurred
        n_children = max(1, n_children)
        
        # Record children for this female
        children_per_female[i] = n_children
        total_children_this_gen += n_children
        
        #add offspring to index of male
        offspring[matesM[1]]=offspring[matesM[1]]+n_children
        ...
        for k in 1:n_children
          ...
          if fcount > max_total_offspring || mcount > max_total_offspring
            break  # Stop if we've exceeded allocated space
          end
          #get dad's index for make_gamete function
         dad=matesM[1]
         if k > n_children/2 #make females (roughly half)
           #make egg
           egg=make_gamete(pgf,mgf,i)
           #make sperm
           sperm=make_gamete(pgm,mgm,dad)
           #save paternal genome for female
           if fcount <= max_total_offspring
             pgf2[fcount,:,:]=sperm
        ...
        # Calculate average compatibility across all mates
        avg_compat = mean([compatibility(mphens[m,2], fphens[i,1]) for m in matesM])
        
        # Calculate expected number of children based on average compatibility and child model
        expected_children = offspring_mean(avg_compat, child_model)
        ...
        ferts=wsample(matesM,probm,n_children,replace=true)  # allow replacement since multiple children
    ...
    # Count actual number of offspring produced
    # fcount and mcount start at 1, so total is (fcount-1) + (mcount-1) = fcount + mcount - 2
    actual_offspring = fcount + mcount - 2
    # Safety check: ensure we don't exceed allocated space
    actual_offspring = min(actual_offspring, max_total_offspring)
    
    # If no offspring produced, population collapses - fill with zeros
    if actual_offspring <= 0
      # Population collapsed - fill results with NaN/zeros for remaining generations
      for remaining_gen in gen:generations
        dfall[remaining_gen,:] = [NaN for _ in 1:24]
        dfall[remaining_gen,21] = remaining_gen  # generation number
      end
      return dfall
    end
    
    # Cap actual_offspring to allocated array size (safety check)
    actual_offspring = min(actual_offspring, max_total_offspring)
    actual_offspring = min(actual_offspring, size(mgf2, 1), size(pgf2, 1), size(mgm2, 1), size(pgm2, 1))
    
    # Trim arrays to actual number of offspring produced
    # Use safe indexing to avoid bounds errors and ensure we create copies, not views
    if actual_offspring > 0 && actual_offspring <= size(mgf2, 1)
      # Create new properly-sized arrays by copying
      mgf2 = copy(mgf2[1:actual_offspring,:,:])
      pgf2 = copy(pgf2[1:actual_offspring,:,:])
      mgm2 = copy(mgm2[1:actual_offspring,:,:])
      pgm2 = copy(pgm2[1:actual_offspring,:,:])
    end
    
    # Sample N individuals from offspring to maintain fixed population size
    if actual_offspring >= N
      # More offspring than needed - sample randomly
      sample_indices = sample(1:actual_offspring, N, replace=false)
      mgf2 = copy(mgf2[sample_indices,:,:])
      pgf2 = copy(pgf2[sample_indices,:,:])
      mgm2 = copy(mgm2[sample_indices,:,:])
      pgm2 = copy(pgm2[sample_indices,:,:])
    else
      # Fewer offspring than needed - sample with replacement
      sample_indices = sample(1:actual_offspring, N, replace=true)
      mgf2 = copy(mgf2[sample_indices,:,:])
      pgf2 = copy(pgf2[sample_indices,:,:])
      mgm2 = copy(mgm2[sample_indices,:,:])
      pgm2 = copy(pgm2[sample_indices,:,:])
    ...
    # Ensure offspring array matches male population size
    n_males = size(mphens, 1)
    if length(offspring) < n_males
      # Pad offspring array if needed
      offspring_padded = zeros(n_males)
      offspring_padded[1:length(offspring)] = offspring
      offspring = offspring_padded
    elseif length(offspring) > n_males
      # Trim if needed
      offspring = offspring[1:n_males]
    end
    ...
    #calculate mean children per female
    MeanChildren = mean(children_per_female[children_per_female .> 0])  # only count females that had children
    if isnan(MeanChildren)
      MeanChildren = 0.0
    end
    
    # Total children born this generation
    TotalChildren = total_children_this_gen

    #put all model results together
    #mean male,mean female, std male, std female,cor,sperm count, sperm count std,is,int,beta,gamma,A,MeanRSC,a,gen,MeanMates,MeanChildren,TotalChildren
    sumdf=[mean(mphens[:,2]),mean(fphens[:,1]),std(mphens[:,2]),std(fphens[:,1]),cor(mphens[:,2],fphens[:,1]),Meansperm,Stdsperm,is,coef(model)[1],coef(model)[2],coef(model)[3],coef(model)[4],coef(model)[5],coef(model)[6],coef(model)[7],coef(model)[8],coef(model)[9],coef(model)[10],a,MeanRSC,gen,MeanMates,MeanChildren,TotalChildren]
    dfall[gen,:]=sumdf
    
    #next generation - update genomes (already sampled to size N)
    mgf = mgf2
    pgf = pgf2
    mgm = mgm2
    pgm = pgm2
    ...
    for i in 1:size(pgm,1)
      for j in 1:size(pgm,2)
        for k in 1:ntraits
          if k == 4  # RSC trait
            pgm[i,j,k] = mutate_rsc(pgm[i,j,k])
            mgm[i,j,k] = mutate_rsc(mgm[i,j,k])
            pgf[i,j,k] = mutate_rsc(pgf[i,j,k])
            mgf[i,j,k] = mutate_rsc(mgf[i,j,k])
          else  # Other traits (1-3): NO CLAMPING - allow natural evolution
            pgm[i,j,k] = mutate(pgm[i,j,k])
            mgm[i,j,k] = mutate(mgm[i,j,k])
            pgf[i,j,k] = mutate(pgf[i,j,k])
            mgf[i,j,k] = mutate(mgf[i,j,k])
          end
        end
      end
    end
    
    # Recalculate phenotypes for next generation (after mutations)
    # This will be used at the start of the next generation
    mphens=reduce(hcat,[sum(pgm[:,:,i],dims=2).+sum(mgm[:,:,i],dims=2) for i in 1:ntraits])
    fphens=reduce(hcat,[sum(pgf[:,:,i],dims=2).+sum(mgf[:,:,i],dims=2) for i in 1:ntraits])
  end
  return(dfall)
end

#function or run simulation so I can put it in a for loop below
@everywhere function runsim(reps,N,mu,var,a,rsc,tradeoff,gens,d=-1,child_model::Symbol=:parabola)
  resultsP=SharedArray{Float64}(reps*gens,25)
  @sync @distributed for i in 1:reps
    @async resultsP[(1+(i-1)*gens):(gens*i),1:24]=sim(N,mu,var,a,rsc,tradeoff,gens,d,child_model)
    @async resultsP[(1+(i-1)*gens):(gens*i),25]=fill(i,gens)
  end
  return(resultsP)
end

# Single-threaded runner (copied/adapted from noeverywhere.jl)
function runsim_serial(reps,N,mu,var,a,rsc,tradeoff,gens,d=-1,child_model::Symbol=:parabola)
  resultsP=zeros(Float64, reps*gens, 25)
  for i in 1:reps
    println("Running replicate $i of $reps with child_model=$child_model...")
    resultsP[(1+(i-1)*gens):(gens*i),1:24]=sim(N,mu,var,a,rsc,tradeoff,gens,d,child_model)
    resultsP[(1+(i-1)*gens):(gens*i),25]=fill(i,gens)
  end
  return(resultsP)
end
  ...
  for j in [true,false]
    for k in [1,12.5,50]
      for l in [0.25,0.5,0.75,1]
        for child_mod in [:parabola, :asymptote, :linear]
          results=runsim(10,500,mu,var,k,l,j,gens,-1,child_mod)
          outdir = "Results_HighVar_20_1000_RSC_Child"
          isdir(outdir) || mkdir(outdir)
          outfile = joinpath(outdir, string("HV_20_1000_",j,"_",k,"_",l,"_",child_mod,".csv"))
...
if get(ENV, "RUN_SINGLE_TEST", "0") == "1"
  ...
  a_test = 1
  rsc_test = 0.25
  child_model_test = :parabola
  results = runsim_serial(5, 100, mu_test, var_test, a_test, rsc_test, tradeoff_test, gens_test, -1, child_model_test)
  data = DataFrame(results, [:MeanMale,:MeanFemale,:SDMale,:SDFemale,:cor,:MeanCount,:SDCount,:is,:int,:BMale,:GMale,:BFemale,:GFemale,:BSperm,:GSperm,:GMF,:GMS,:GFS,:a,:MeanRSC,:Generation,:MeanMates,:MeanChildren,:TotalChildren,:Rep])
